...
- Loop backend, liquidación y procesamiento de pagos confirmados: nueva migración `20260924031643_guardian_settlement.sql` vincula la evidencia a la factura/ciclo persistidos, reduce la reserva al neto real de forma atómica y devuelve el bruto completo si la reserva ya no es válida. Las asignaciones confirmadas ocupan capacidad junto a las aportaciones individuales y aparecen en los totales públicos. Cola privada con arrendamiento, reintentos limitados, claves Stripe estables, transferencias por gasto y devolución completa; recuperación de respuestas perdidas de Stripe y PostgreSQL. `payment-worker` integra el procesador mediante `DOPMI_GUARDIAN_WORKER_ENABLED=true`, desactivado por defecto. **102/102 pruebas locales** aprobadas con PGlite y Stripe simulado, incluidas cuentas destino cambiadas, importes de transferencia incorrectos, devolución parcial existente y múltiples asignaciones. `deno check` del trabajador aprobado usando las dependencias fijadas en un entorno temporal aislado. Prueba de concurrencia PostgreSQL ampliada para liquidación contra Checkout individual; pendiente de ejecución en CI. La revisión automática bloqueó publicar el nuevo conjunto de archivos en GitHub por considerar insuficiente la autorización de exportación; queda pendiente aprobación explícita para el código/documentación de este loop. Aún no se ejecutó CI para este cambio, no se desplegó esta migración ni se habilitó el trabajador. La creación y el cobro inicial/mensual de suscripciones, devoluciones posteriores a transferencias, interfaz y aceptación Stripe integral siguen pendientes; este avance procesa facturas pagadas y previamente vinculadas.
