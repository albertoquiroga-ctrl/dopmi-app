...
- Reserva transaccional propuesta en `20260923224118_guardian_atomic_reservations.sql`: ciclo privado por donante y clave estable, asignaciones privadas, capacidad calculada después de adquirir bloqueos por rescatista en orden, vencimiento a los 30 minutos y liberación idempotente. La capacidad individual y la lectura pública del disponible ahora descuentan las reservas vigentes. Solo `service_role` puede reservar o liberar; no hay endpoint móvil ni cobro Stripe. Se reserva el límite superior del neto (bruto menos comisión Dopmi) porque la comisión real de Stripe aún se desconoce. La selección debe revalidarse antes de cualquier cobro futuro; falta integración, aceptación CI/real y prueba específica de concurrencia en PostgreSQL real.
