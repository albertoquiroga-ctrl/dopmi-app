...
Este paso está protegido adicionalmente por `DOPMI_GUARDIAN_SCHEDULE_ENABLED=true`, junto con el flag del trabajador y la clave de prueba. Todavía no paga facturas, no habilita un endpoint de alta en la app y no implementa solicitudes de cambio/cancelación desde el cliente. El comportamiento de creación protegida requiere aceptación con Stripe real de prueba antes de habilitar el recorrido. Las pruebas locales usan Stripe simulado; la prueba de concurrencia usa PostgreSQL real en CI. Stripe Tax continúa desactivado en este alcance.
