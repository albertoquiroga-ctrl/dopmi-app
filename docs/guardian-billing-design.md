...
El webhook firmado atiende sesiones conocidas de Guardián y el trabajador recupera altas pendientes bajo `DOPMI_GUARDIAN_WORKER_ENABLED`. El código no expone todavía un endpoint de alta para la app ni crea el calendario de Billing. Un pago inicial asignado queda en `funded_pending_schedule`, no se presenta como suscripción activa. Antes de exponerlo faltan crear y verificar el calendario mensual retenido en borrador, vincularlo con esta evidencia, implementar cobro condicionado/cambio/cancelación y completar la aceptación de Stripe. Stripe Tax sigue desactivado conforme al alcance de prueba existente.
