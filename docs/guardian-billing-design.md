...
- Con capacidad: el servidor crea una reserva idempotente y un Checkout de suscripción en modo de prueba. Una sesión abandonada o vencida libera la reserva; una factura inicial sin pago no se presenta como aportación confirmada.
...
3. Para cada renovación, probar en sandbox el mecanismo que impide realmente cobrar la factura si falta capacidad. No confiar sólo en `invoice.upcoming`, `invoice.created`, una pausa tardía o un webhook que responde 200. Cubrir factura inicial, reintentos, entrega duplicada o desordenada, bloqueo de concurrencia, cambio de monto, cancelación y conciliación tras pérdida de respuesta.
