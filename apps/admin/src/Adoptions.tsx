...
export default function Adoptions({ api }: { api: AdminApi }) {
  ...
  return <>
    ...
    <section className="users-card"><div className="table-toolbar"><div><h2>Cola de publicaciones</h2><p aria-live="polite">{loading ? 'Consultando…' : `${data.total} ${data.total === 1 ? 'publicación' : 'publicaciones'}`}</p></div>
